...
const buildBiomeCommand = (filenames) => {
	...
	return `pnpm --filter website exec biome check --write --no-errors-on-unmatched --files-ignore-unknown=true --colors=off ${files}`;
};

export default {
	// Typecheck TS/TSX before commit (same as before)
	"**/*.{ts,tsx}": () => "pnpm --filter website exec tsc --noEmit",
